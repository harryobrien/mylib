...
export default function ReadingProgress({ slug }: Props) {
  ...
  const { data: editions } = useSWR(user ? "userEditions" : null, fetchUserEditions, {
          ...
          <span className="progress-label">
            {editingPage ? (
              ...
              <button
                ...
                onClick={() => {
                  setPageValue(String(current_page || 0));
                  setEditingPage(true);
                }}
              >
                {current_page || 0}
          ...
          <span className="reading-date">
            ...
            {editingStart ? (
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter")
                    saveDate("started_at", (e.target as HTMLInputElement).value);
              ...
              <button
                type="button"
                className="date-btn date-btn-empty"
          ...
          <span className="reading-date">
            ...
            {editingFinish ? (
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter")
                    saveDate("finished_at", (e.target as HTMLInputElement).value);
